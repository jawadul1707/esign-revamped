...
class FaceLivenessScreen extends StatefulWidget {
  ...
  const FaceLivenessScreen({super.key, required this.cameras});
...
class _FaceLivenessScreenState extends State<FaceLivenessScreen> {
  ...
  bool isLoading = false; // For showing the CircularProgressIndicator
  ...
  void _uploadImages() async {
    ...
    try {
      // Check if the image paths are null or empty
      if (clearImageLocation == null ||
          clearImageLocation!.isEmpty ||
          capturedPhotoPath == null ||
          capturedPhotoPath!.isEmpty) {
      ...
      var request = http.MultipartRequest(
        'POST',
        Uri.parse(
            'https://staging-gw.e-sign.com.bd:9100/user-management-service/api/v1/face/recognition'),
      );

      request.files.add(await http.MultipartFile.fromPath(
          'sourceImage', clearImageLocation!));
      request.files.add(
          await http.MultipartFile.fromPath('targetImage', capturedPhotoPath!));
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonResponse['statusCode'] == "OK") {
          ...
          Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => const InvalidPage()),
          );
        }
      } else {
        // Show snackbar for a server error
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
      ...
      debugPrint("Error: $e");
      // Show snackbar for an exception
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            ...
            children: [
              ...
              Padding(
                ...
                child: LinearProgressIndicator(
                  value: (currentInstructionIndex == instructions.length - 1 &&
                          !isWaiting)
                      ? 1.0
            ...
            child: ElevatedButton(
              ...
              child: isLoading
                  ? const CircularProgressIndicator(
                      valueColor:
                          AlwaysStoppedAnimation<Color>(Color(0xFF005D99)),
                    )
                  : const Text(
                      "Continue",
